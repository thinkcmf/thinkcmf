...
class ThemeLogic
{
    ...
    public static function getList()
    {
        ...
        $themeManifestsIntalled = OptionModel::where('option_name', 'like', "app_manifest_%")->select();

        $themesIntalled = [];
        foreach ($themeManifestsIntalled as $themeManifestIntalled) {
            $themesIntalled[$themeManifestIntalled['option_name']] = $themeManifestIntalled['option_value'];
        }

        if (empty($dirs)) return $themes;


        foreach ($dirs as $themeName) {
            $themePath    = app_path() . $themeName . DIRECTORY_SEPARATOR;
            $manifestFile = $themePath . 'manifest.json';
            ...
            if (!$formatWrong) {
                $themeInfo = [];
                if (!empty($themesIntalled["app_manifest_{$themeName}"])) {
                    $themeInfo                  = $themesIntalled["app_manifest_{$themeName}"];
                    $themeInfo['installed']     = 1;
                    $themeInfo['local_verison'] = $manifest['version'];
                } else {
                    $themeInfo                  = $manifest;
                    $themeInfo['local_verison'] = $manifest['version'];
                }
