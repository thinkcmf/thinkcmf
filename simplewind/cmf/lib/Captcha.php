...
class Captcha
{
    ...
    public function entry($id = '')
    {
        ...
        if (empty($this->fontttf)) {
            ...
            while (false !== ($file = $dir->read())) {
                if ('.' != $file[0] && substr($file, -4) == '.ttf' && $file != '6.ttf') {
                    $ttfs[] = $file;
                }
            }
            $dir->close();
        ...
        if ($this->useZh) {
            ...
            $mIndex = mt_rand(1, 100);

            if ($mIndex <= mt_rand(1, 100)) {
                ...
                $mCode[1] = $operatorStrArr[$operator];
                $mCode[3] = '=';
                $mCode[4] = '?';
                ...
                if ($operator == 2) {
                    ...
                    $mCode[0] = $mCode0Arr[mt_rand(0, 3)];
                    $mCode[2] = $mCode2Arr[mt_rand(0, 1)];
                    $code[]   = $mCode[0] / $mCode[2];
                } else {
                    $mCode[0] = mt_rand(0, 9);
                    $mCode[2] = mt_rand(1, 9);
                    switch ($operator) {
    ...
    {
        $path = VENDOR_PATH . 'topthink/think-captcha/assets/bgs/';
        $dir  = dir($path);
